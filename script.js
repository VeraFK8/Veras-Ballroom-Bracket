...
resetPasswordForm?.addEventListener('submit', async (event) => {
  ...
  showAuthMessage('Sending password reset link...');
  ...
  if (error) {
    showAuthMessage(error.message || 'Could not send reset email.', true);
    return;
  }

  showAuthMessage('♡ Reset link sent! Check your email.');
});
...
function parseLikelyScore(line) {
  // Prefer explicit score language, e.g. "score 36", "36/40", "total: 36".
  const explicit = line.match(/(?:score|total|judges?|points?)\s*[:\-]?\s*(\d{1,3}(?:\.\d+)?)/i);
...
function parseImportedResultsText(text) {
  ...
  lines.forEach((line) => {
    const couple = findCoupleMentionInLine(line);
    if (!couple) {
      unmatched.push(line);
      return;
    }
    ...
    const existing = parsedByCouple.get(key) || {
      couple_id: key,
      couple,
      judges_score: null,
    ...
    const n = normalizeImportText(line);
    const score = parseLikelyScore(line);
    if (score !== null) existing.judges_score = score;

    if (/\bperfect\b/.test(n) || /\bperfect score\b/.test(n)) existing.got_perfect_score = true;
    if (/\bfirst 10\b/.test(n) || /\bfirst ten\b/.test(n)) existing.got_first_10_of_season = true;
    if (/\bchallenge winner\b/.test(n) || /\bchallenge win\b/.test(n) || /\brelay winner\b/.test(n) || /\brelay win\b/.test(n) || /\bdance off winner\b/.test(n)) existing.challenge_win = true;
    if (/\bsafe\b/.test(n) && !/\bunsafe\b/.test(n)) existing.was_safe = true;
    if (/\bbottom\b/.test(n) || /\bbottom group\b/.test(n) || /\bbottom two\b/.test(n) || /\bbottom 2\b/.test(n)) existing.was_bottom_group = true;
    if (/\beliminated\b/.test(n) || /\bsent home\b/.test(n) || /\bwent home\b/.test(n) || /\bout of the competition\b/.test(n)) existing.was_eliminated = true;
    if (/\bsemifinal\b/.test(n) || /\bsemi final\b/.test(n) || /\bsemis\b/.test(n)) existing.made_semifinals = true;
    if (/\bfinale\b/.test(n) || /\bfinalist\b/.test(n)) existing.made_finale = true;

    if (/\bwinner\b/.test(n) && !/\bchallenge winner\b/.test(n) && !/\brelay winner\b/.test(n)) existing.final_placement = 1;
    if (/\brunner up\b/.test(n) || /\bsecond place\b/.test(n) || /\b2nd place\b/.test(n)) existing.final_placement = 2;
    if (/\bthird place\b/.test(n) || /\b3rd place\b/.test(n)) existing.final_placement = 3;

    // Eliminated is not also "safe" for scoring purposes.
    if (existing.was_eliminated) existing.was_safe = false;

    existing.source_lines.push(line);
    parsedByCouple.set(key, existing);
  });
...
async function handleAdminImportParse() {
  ...
  if (note) {
    note.textContent = filled
      ? `♡ Auto-filled ${filled} couple${filled === 1 ? '' : 's'}. Review every row before finalizing.`
      : '♡ I could not confidently match any couple rows. Try a cleaner score list.';
  }
...
async function runAdminReset(kind) {
  ...
  const rpcName = isFull
    ? 'admin_reset_gameplay'
    : 'admin_reset_scores_and_results';
  ...
  if (typed.trim() !== phrase) {
    if (note) note.textContent = `♡ Reset canceled — confirmation phrase did not match "${phrase}".`;
    return;
  }
  ...
  if (error) {
    ...
    if (note) note.textContent = `♡ Reset failed: ${error.message}`;
    return;
  }
  ...
  if (note) {
    note.textContent = isFull
      ...
      : '♡ Test scores and results reset complete.';
  }

  // Refresh all admin/account surfaces that may have changed.
  await loadSeasonSettings();
  await loadAdminWeek();
  await loadSeasonStageResults();
  await loadScoreBreakdown();
  await loadLeaderboard();
  ...
  if (isFull) {
    await loadSavedTeam();
    await loadWeeklyPicksForCurrentWeek();
    await loadBracketPicks();
  }
}
