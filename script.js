...
function handleFantasyTeamSelectionChange(slotNumber) {
  const changed = document.querySelector(`#couple-${slotNumber}`);
  if (!changed) return;

  const chosen = changed.value;

  if (chosen) {
    const duplicateElsewhere = [1,2,3,4]
      .filter((n) => n !== slotNumber)
      .map((n) => document.querySelector(`#couple-${n}`)?.value)
      .some((value) => value === chosen);

    // Extra safety in case a browser lets a disabled option be re-selected
    // from a stale dropdown state.
    if (duplicateElsewhere) {
      changed.value = '';
      renderCouplePreview(slotNumber);
      updateFantasyTeamDuplicateOptions();
      if (teamNote) teamNote.textContent = '♡ That couple is already on your team. Choose a different couple.';
      return;
    }
  }
...
function updateTeamLockUI(saved = currentTeamRow) {
  ...
  if (rerollAvailable) {
    ...
    if (teamLockCopy) {
      teamLockCopy.textContent =
        `You have one reroll. Change any of your four couples now, then lock your final lineup. Your new lineup starts scoring in Week ${CURRENT_WEEK}. Using it gives up the +${LOYALTY_BONUS_POINTS} loyalty bonus. If you keep your original team, that bonus stays pending and is awarded at the end of the season.`;
    }
    return;
  }
  ...
  if (finalLocked) {
    ...
    if (teamLockCopy) {
      teamLockCopy.textContent =
        `Your original four are locked through Week ${REROLL_OPEN_AFTER_WEEK}. Keep them all season to earn the +${LOYALTY_BONUS_POINTS} loyalty bonus. The bonus is awarded when the season ends.`;
    }
  }
}
...
[1,2,3,4].forEach((n) => {
  ...
  if (select) {
    select.addEventListener('change', () => {
      ...
      handleFantasyTeamSelectionChange(n);
    });
  }
});
...
async function fetchRecapFromUrl() {
  ...
  if (error) {
    ...
    if (note) {
      note.innerHTML = `
        ♡ This website blocked automatic fetching or the page could not be read.
        <br><strong>Fallback:</strong> copy the recap text from the article, paste it into the box below,
        then click <strong>AUTO-FILL RESULTS ♡</strong>.
      `;
    }
  ...
  if (!data?.text) {
    if (note) {
      note.innerHTML = `
        ♡ I reached the page, but could not extract readable recap text.
        <br><strong>Fallback:</strong> copy the recap text from the article, paste it into the box below,
        then click <strong>AUTO-FILL RESULTS ♡</strong>.
      `;
    }
...
function importNameParts(value) {
...
function coupleImportAliases(couple) {
  const celebrityRaw = String(
    couple.celebrity_name ?? couple.celebrity ?? couple.star_name ?? couple.star ?? ''
  ).trim();
  const proRaw = String(
    couple.pro_name ?? couple.pro ?? couple.dancer_name ?? couple.dancer ?? ''
  ).trim();

  const celebrity = importNameParts(celebrityRaw);
  const pro = importNameParts(proRaw);

  const aliases = [
    // Strongest: both people together.
    `${celebrity.full} and ${pro.full}`,
    `${celebrity.full} ${pro.full}`,
    `${celebrity.first} and ${pro.first}`,
    `${celebrity.first} ${pro.first}`,

    // Full individual names.
    celebrity.full,
    pro.full,

    // Single first/last names are useful for recap prose, but are only used
    // when that alias belongs to exactly one current couple (see below).
    celebrity.first,
    celebrity.last,
    pro.first,
    pro.last
...
function buildImportAliasMap() {
  ...
  dedupeCouples(couples).forEach((couple) => {
    ...
    // Pair aliases are always strongest.
    add(`${celebrity.full} and ${pro.full}`, couple, 100, 'pair');
    add(`${celebrity.full} ${pro.full}`, couple, 95, 'pair');
    add(`${celebrity.first} and ${pro.first}`, couple, 90, 'pair');
    add(`${celebrity.first} ${pro.first}`, couple, 85, 'pair');

    // Full individual names.
    add(celebrity.full, couple, 80, 'celebrity');
    add(pro.full, couple, 75, 'pro');

    // Short names. A celebrity first name is intentionally preferred over a
    // pro first name when the same first name exists in both roles. Recaps
    // overwhelmingly use the contestant/celebrity's first name as shorthand.
    add(celebrity.first, couple, 60, 'celebrity');
    add(celebrity.last, couple, 55, 'celebrity');
    add(pro.first, couple, 45, 'pro');
    add(pro.last, couple, 40, 'pro');
  });
...
function findCoupleMentionInLine(line) {
  ...
  const aliasMap = buildImportAliasMap();
  const paddedLine = ` ${normalized} `;
  const candidates = [];

  aliasMap.forEach((entries, alias) => {
    const paddedAlias = ` ${alias} `;
    if (!paddedLine.includes(paddedAlias)) return;

    entries.forEach((entry) => {
      candidates.push({
        ...entry,
        alias,
        score: entry.strength + Math.min(alias.length, 40)
      });
    });
  });
  ...
  // Stronger/longer aliases win first.
  candidates.sort((a, b) => b.score - a.score);

  const topScore = candidates[0].score;
  const top = candidates.filter((c) => c.score === topScore);

  // Exact tie for the same couple is harmless.
  const uniqueTopCouples = [...new Set(top.map((c) => String(c.couple.id)))];
  if (uniqueTopCouples.length === 1) return top[0].couple;

  // Special ambiguity rule:
  // if a short first name is shared by a celebrity and a pro on different
  // couples, prefer the celebrity's couple. Example: celebrity Ezra vs pro Ezra.
  const celebTop = top.filter((c) => c.role === 'celebrity');
  const celebCouples = [...new Set(celebTop.map((c) => String(c.couple.id)))];
  if (celebCouples.length === 1) return celebTop[0].couple;
...
async function changeSeasonWeek(delta) {
  ...
  await loadAdminWeek();
  await loadPriorEliminationsForAdminWeek();
  await loadWeeklyPicksFromSupabase();
...
// ---------------- ADMIN PLAYER SUBMISSION LOG ----------------
let adminSubmissionLogRows = [];
let adminSubmissionPlayers = new Map();

function submissionTypeLabel(type) {
  return {
    fantasy_team: 'FANTASY TEAM',
    weekly_picks: 'WEEKLY PICKS',
    bracket_pick: 'BRACKET PICK',
    bracket_submission: 'BRACKET LOCK'
  }[type] || String(type || 'SUBMISSION').toUpperCase();
}

function readableCoupleName(id) {
  if (!id) return '—';
  const couple = findCouple(id);
  return couple ? coupleLabel(couple) : String(id).slice(0, 8);
}

function formatSubmissionSnapshot(row) {
  const data = row.new_data || row.old_data || {};

  if (row.submission_type === 'weekly_picks') {
    return [
      `<b>Highest:</b> ${readableCoupleName(data.highest_scoring_couple_id)}`,
      `<b>Lowest:</b> ${readableCoupleName(data.lowest_scoring_couple_id)}`,
      `<b>Eliminated:</b> ${readableCoupleName(data.eliminated_couple_id)}`,
      `<b>Perfect score:</b> ${data.perfect_score === true ? 'Yes' : data.perfect_score === false ? 'No' : '—'}`
    ].join(' • ');
  }

  if (row.submission_type === 'fantasy_team') {
    const team = [
      data.couple_1_id,
      data.couple_2_id,
      data.couple_3_id,
      data.couple_4_id
    ].filter(Boolean).map(readableCoupleName);

    const reroll = data.reroll_used
      ? ` • <b>Reroll:</b> Yes${data.reroll_week ? ` (Week ${data.reroll_week})` : ''}`
      : ' • <b>Reroll:</b> No';

    return `<b>Lineup:</b> ${team.length ? team.join(' / ') : '—'}${reroll}`;
  }

  if (row.submission_type === 'bracket_pick') {
    return [
      `<b>Stage:</b> ${String(data.stage || '—').replace(/_/g, ' ').toUpperCase()}`,
      `<b>Pick:</b> ${readableCoupleName(data.couple_id)}`,
      data.pick_order != null ? `<b>Order:</b> ${data.pick_order}` : ''
    ].filter(Boolean).join(' • ');
  }

  if (row.submission_type === 'bracket_submission') {
    return `<b>Bracket locked:</b> ${data.locked_at ? new Date(data.locked_at).toLocaleString() : 'Yes'}`;
  }

  return 'Submission recorded.';
}

function renderAdminSubmissionLog() {
  const list = document.getElementById('admin-submission-log-list');
  const note = document.getElementById('admin-submission-log-note');
  if (!list) return;

  const playerFilter = document.getElementById('admin-log-player-filter')?.value || '';
  const typeFilter = document.getElementById('admin-log-type-filter')?.value || '';
  const weekFilter = document.getElementById('admin-log-week-filter')?.value || '';

  const filtered = adminSubmissionLogRows.filter((row) => {
    if (playerFilter && String(row.user_id) !== playerFilter) return false;
    if (typeFilter && row.submission_type !== typeFilter) return false;
    if (weekFilter && String(row.week_number ?? '') !== weekFilter) return false;
    return true;
  });

  if (note) {
    note.textContent = `♡ Showing ${filtered.length} of ${adminSubmissionLogRows.length} saved submission log entr${adminSubmissionLogRows.length === 1 ? 'y' : 'ies'}.`;
  }

  if (!filtered.length) {
    list.innerHTML = '<div class="admin-log-empty-v62">No submission history matches those filters yet.</div>';
    return;
  }

  list.innerHTML = filtered.map((row) => {
    const player = adminSubmissionPlayers.get(String(row.user_id)) || {};
    const displayName = player.display_name || 'Unknown Player';
    const teamName = player.team_name || 'No team name';
    ...
    return `
      <article class="admin-log-entry-v62">
        <div class="admin-log-time-v62">${timeText}</div>
        <div class="admin-log-player-v62">
          <strong>${displayName}</strong>
          <small>${teamName}</small>
        </div>
        <div class="admin-log-detail-v62">
          <div class="admin-log-badges-v62">
            <span>${submissionTypeLabel(row.submission_type)}</span>
            <span>${String(row.action || '').toUpperCase()}</span>
            ${row.week_number ? `<span>WEEK ${row.week_number}</span>` : ''}
          </div>
          <div class="admin-log-summary-v62">${formatSubmissionSnapshot(row)}</div>
        </div>
      </article>
    `;
  }).join('');
...
async function loadAdminSubmissionLog() {
  const list = document.getElementById('admin-submission-log-list');
  const note = document.getElementById('admin-submission-log-note');
  const button = document.getElementById('admin-refresh-submission-log');
  ...
  if (note) note.textContent = 'Loading player submission history...';

  const [{ data: logs, error: logError }, { data: players, error: playerError }] = await Promise.all([
    db
      .from('player_submission_log')
      .select('id, user_id, submission_type, action, week_number, record_id, old_data, new_data, recorded_at')
  ...
  if (logError) {
    console.error('Could not load submission log:', logError);
    list.innerHTML = '<div class="admin-log-empty-v62">Submission log unavailable.</div>';
    if (note) {
      note.textContent = logError.message?.includes('player_submission_log')
        ? '♡ The submission-log SQL still needs to be run in Supabase.'
        : `♡ Could not load the log: ${logError.message}`;
    }
    return;
  }

  if (playerError) {
    console.warn('Could not load player names for submission log:', playerError);
  }

  adminSubmissionLogRows = logs || [];
  adminSubmissionPlayers = new Map(
    (players || []).map((player) => [String(player.id), player])
  );

  const playerSelect = document.getElementById('admin-log-player-filter');
  if (playerSelect) {
    const current = playerSelect.value;
    const userIds = [...new Set(adminSubmissionLogRows.map((row) => String(row.user_id)))];

    playerSelect.innerHTML = '<option value="">All players</option>' +
      userIds.map((id) => {
        const player = adminSubmissionPlayers.get(id) || {};
        const label = player.display_name
          ? `${player.display_name}${player.team_name ? ` — ${player.team_name}` : ''}`
          : `Player ${id.slice(0, 8)}`;
        return `<option value="${id}">${label}</option>`;
      }).join('');

    if ([...playerSelect.options].some((o) => o.value === current)) {
      playerSelect.value = current;
    }
  }

  renderAdminSubmissionLog();
}
...
document.getElementById('admin-fetch-url')
  ...
  document.getElementById('admin-refresh-submission-log')
    ?.addEventListener('click', loadAdminSubmissionLog);

  document.getElementById('admin-log-player-filter')
    ?.addEventListener('change', renderAdminSubmissionLog);

  document.getElementById('admin-log-type-filter')
    ?.addEventListener('change', renderAdminSubmissionLog);

  document.getElementById('admin-log-week-filter')
    ?.addEventListener('change', renderAdminSubmissionLog);
...
let adminPriorEliminatedIds = new Set();

async function loadPriorEliminationsForAdminWeek() {
  const week = selectedAdminWeek();
  const note = document.getElementById('admin-workflow-note');
  ...
  if (error) {
    console.error('Could not load prior eliminations:', error);
    adminPriorEliminatedIds = new Set();
    if (note) note.textContent = '♡ Could not check prior eliminations. Review the week carefully before finalizing.';
    return;
  }

  adminPriorEliminatedIds = new Set((data || []).map((row) => String(row.couple_id)));
  applyAdminPriorEliminationLocks();

  if (note) {
    note.textContent = adminPriorEliminatedIds.size
      ? `♡ ${adminPriorEliminatedIds.size} previously eliminated couple${adminPriorEliminatedIds.size === 1 ? '' : 's'} locked out for Week ${week}.`
      : `♡ No couples were eliminated before Week ${week}.`;
  }
}

function applyAdminPriorEliminationLocks() {
  document.querySelectorAll('#admin-results-body tr').forEach((tr) => {
    const locked = adminPriorEliminatedIds.has(String(tr.dataset.coupleId));
    tr.classList.toggle('admin-row-eliminated-v56', locked);
    ...
    if (locked) {
      const rank = tr.querySelector('.admin-rank');
      if (rank) rank.textContent = 'OUT';
    }
  });
}
...
async function saveAdminWeek(finalize) {
  ...
  if (finalize) {
    const activeRows = rows.filter((r) => !adminPriorEliminatedIds.has(String(r.couple_id)));
    const missingScores = activeRows.filter((r) => r.judges_score === null);

    if (missingScores.length) {
      const names = missingScores
        .slice(0, 5)
        .map((r) => coupleNameById(r.couple_id))
        .join(', ');
      const more = missingScores.length > 5 ? ` + ${missingScores.length - 5} more` : '';

      const continueWithoutScores = window.confirm(
        `Week ${week} still has ${missingScores.length} active couple${missingScores.length === 1 ? '' : 's'} without a judges score:\n\n${names}${more}\n\nFinalize anyway?`
      );

      if (!continueWithoutScores) {
        if (note) note.textContent = '♡ Finalize canceled. Add or review the missing scores first.';
        return;
      }
    }

    const highest = chooseSinglePredictionCouple(enteredRows, 'highest');
    const lowest = chooseSinglePredictionCouple(enteredRows, 'lowest');
    const perfect = enteredRows.some((r) => r.got_perfect_score);
    const eliminatedNames = eliminatedRows.length
      ? eliminatedRows.map((r) => coupleNameById(r.couple_id)).join(' / ')
      : 'None';

    const confirmed = window.confirm(
      `FINALIZE WEEK ${week}?\n\nHighest: ${coupleNameById(highest)}\nLowest: ${coupleNameById(lowest)}\nEliminated: ${eliminatedNames}\nPerfect score: ${perfect ? 'YES' : 'NO'}\n\nThis will recalculate everybody's points and update the leaderboard.`
    );

    if (!confirmed) {
      if (note) note.textContent = '♡ Finalize canceled. Nothing was scored.';
      return;
    }
  }
  ...
  if (finalize) {
    ...
    const workflowNote = document.getElementById('admin-workflow-note');
    if (workflowNote) workflowNote.textContent = `♡ Week ${week} is scored. Verify the leaderboard, then use ADVANCE WEEK when you're ready.`;
    await loadLeaderboard();
    await loadEliminatedCouples();
    await loadScoreBreakdown();
  } else {
