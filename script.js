...
function buildImportAliasMap() {
  ...
  dedupeCouples(couples).forEach((couple) => {
    coupleImportAliases(couple).forEach((alias) => {
      if (alias.length < 3) return;
      if (!aliasMap.has(alias)) aliasMap.set(alias, []);
      aliasMap.get(alias).push(couple);
    });
  });
...
function findCoupleMentionInLine(line) {
  ...
  aliasMap.forEach((matchedCouples, alias) => {
    // Do not trust ambiguous short aliases. Example: if two cast members share
    // "Emma", a recap saying only "Emma" will not be auto-assigned.
    if (matchedCouples.length !== 1) return;

    // Word-boundary-ish check after normalization to avoid partial-name matches.
    const paddedLine = ` ${normalized} `;
    const paddedAlias = ` ${alias} `;
    if (!paddedLine.includes(paddedAlias)) return;

    if (alias.length > bestLength) {
      best = matchedCouples[0];
