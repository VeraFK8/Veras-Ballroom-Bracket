...
// Central Time deadlines. Using an explicit offset for the 2026 season dates.
const INITIAL_TEAM_DEADLINE = new Date('2026-09-15T19:00:00-05:00');
...
async function loadWeeklyPicksFromSupabase() {
  ...
  const { data, error } = await db
    .from('weekly_picks')
    .select('id, user_id, week_number, highest_scoring_couple_id, eliminated_couple_id, perfect_score, locked_at, created_at, updated_at')
    .eq('user_id', user.id)
...
if (picksForm) {
  picksForm.addEventListener('submit', async (event) => {
    ...
    if (!highest || !eliminated || !perfectValue) {
