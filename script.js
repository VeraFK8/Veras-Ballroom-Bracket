...
// ---------------- DYNAMIC IMPORTER TEST ----------------
function loadDynamicTestRecap() {
  ...
  if (!source || cast.length < 7) {
    if (note) note.textContent = '♡ Could not build the test recap because the current cast is not loaded yet.';
    return;
  }

  const label = (couple) => {
    const celebrity = String(couple.celebrity_name ?? couple.celebrity ?? couple.star_name ?? couple.star ?? '').trim();
    const pro = String(couple.pro_name ?? couple.pro ?? couple.dancer_name ?? couple.dancer ?? '').trim();
    if (celebrity && pro) return `${celebrity} and ${pro}`;
    return coupleLabel(couple).replace(/\s*\+\s*/g, ' and ');
  };

  const celebFirst = (couple) => {
    const raw = String(couple.celebrity_name ?? couple.celebrity ?? couple.star_name ?? couple.star ?? '').trim();
    return raw.split(/\s+/)[0] || label(couple);
  };
  const proFirst = (couple) => {
    const raw = String(couple.pro_name ?? couple.pro ?? couple.dancer_name ?? couple.dancer ?? '').trim();
    return raw.split(/\s+/)[0] || '';
  };

  const lines = [
    // Full pair
    `${label(cast[0])} scored 24 points and were safe.`,

    // Celebrity first name only
    `${celebFirst(cast[1])} earned 22 points and were safe.`,

    // Celebrity + pro first names only
    `${celebFirst(cast[2])} and ${proFirst(cast[2])} received 20 points and landed in the bottom group.`,

    // Full pair
    `${label(cast[3])} scored 18 points and were eliminated from the competition.`,

    // Celebrity first name only + perfect score
    `${celebFirst(cast[4])} earned a perfect score of 40 points and were safe.`,

    // Full pair + first 10
    `${label(cast[5])} received the first 10 of the season and scored 28 points.`,

    // Celebrity + pro first names + challenge
    `${celebFirst(cast[6])} and ${proFirst(cast[6])} won the dance-off challenge and scored 26 points.`
  ];

  source.value = lines.join('\n\n');

  if (note) note.textContent = '♡ Test recap loaded using the EXACT couples currently in your database. Auto-filling now...';

  handleAdminImportParse();
}
...
function initializeAdminResultsCenter() {
  ...
  document.getElementById('admin-load-test-recap')
    ?.addEventListener('click', loadDynamicTestRecap);
