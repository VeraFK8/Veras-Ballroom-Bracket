...
function showLoggedIn() {
  ...
  if (profileArea) profileArea.style.display = "grid";
...
if (signupForm) signupForm.addEventListener("submit", async (event) => {
  ...
  signupForm.reset();
  if (data.session) { showAuthMessage("Account created! You\'re signed in."); await loadProfile(); window.scrollTo({ top: 0, behavior: "smooth" }); }
...
async function loadUserTeamFromSupabase() {
  ...
  const { data: saved, error } = await db
    .from('user_teams')
    .select('couple_1_id, couple_2_id, couple_3_id, couple_4_id')
    .eq('user_id', user.id)
    .maybeSingle();
  ...
  if (!saved) {
    ...
    if (teamNote) teamNote.textContent = 'Choose four couples, then save your team ♡';
    return;
  }
  ...
  if (teamNote) teamNote.textContent = '♡ Your saved team is loaded.';
}
...
if (teamForm) {
  teamForm.addEventListener('submit', async (event) => {
    ...
    teamNote.textContent = 'Saving your team...';
    ...
    if (error) {
      ...
      teamNote.textContent = `♡ Could not save your team: ${error.message}`;
      return;
    }
    ...
    renderTeam(team);
    teamNote.textContent = '♡ Your team is saved to your account!';
  });
}
...
if (clearTeamButton) {
  ...
  clearTeamButton.addEventListener('click', () => {
    ...
    [1,2,3,4].forEach((n) => renderCouplePreview(n));
    if (teamNote) teamNote.textContent = 'Form reset. Your saved account team was not deleted.';
  });
}


function renderAccountPicksSummary() {
  if (!accountPicksSummary) return;
  ...
  if (!picks) {
    accountPicksSummary.innerHTML = '<p>No weekly picks saved yet.</p>';
    return;
  }
  ...
  accountPicksSummary.innerHTML = `
    <p><strong>Highest Score:</strong> ${highestCouple ? coupleLabel(highestCouple) : (picks.highest || '—')}</p>
    <p><strong>Eliminated:</strong> ${eliminatedCouple ? coupleLabel(eliminatedCouple) : (picks.eliminated || '—')}</p>
    <p><strong>Perfect Score:</strong> ${picks.perfect || '—'}</p>
  `;
}
